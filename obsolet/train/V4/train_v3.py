...
"""
Deterministic Training Pipeline V3 (Pfad‑B)
...
- Architektur garantiert quantisierbar
"""
...
def representative_dataset():
    for folder in FEATURE_DIRS.values():
        ...
        for f in files[:150]:  # deterministisch, begrenzt
            feat = np.load(os.path.join(folder, f)).astype(np.float32)
            yield [feat.reshape(1, -1)]  # WICHTIG: (1,40)
